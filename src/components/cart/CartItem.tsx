...
export default function CartItem({ item, locale, onRemove }: CartItemProps) {
    ...
    return (
        <div
            className={`flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4 border border-gray-200 rounded-lg hover:border-gray-300 transition-colors ${isRemoving ? 'opacity-50' : ''
                }`}
        >
            {/* Product Image */}
            <div className="flex-shrink-0 w-full sm:w-24 h-32 sm:h-24">
            ...
            <div className="flex-1 flex flex-col gap-3">
                ...
                <div className={`flex-1 ${isArabic ? 'text-right' : 'text-left'}`}>
                    <Link href={`/${locale}/product/${product.slug}`}>
                        <h3 className="font-semibold text-sm sm:text-base text-gray-900 hover:text-blue-600 line-clamp-2">
                            {productName}
                        </h3>
                    </Link>
                ...
                <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 ${isArabic ? 'sm:flex-row-reverse' : ''}`}>
                    {/* Quantity Controls */}
                    <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden bg-white w-fit">
                        <button
                            onClick={() => handleQuantityChange(item.quantity - 1)}
                            className="px-3 py-2 hover:bg-gray-100 text-gray-600 hover:text-gray-900 transition-colors"
                            aria-label={t('cart.decrease_quantity')}
                        >
                            <Minus size={16} />
                        </button>
                        <span className="px-4 py-2 text-center min-w-[2rem] font-semibold text-sm">
                            {item.quantity}
                        </span>
                        <button
                            onClick={() => handleQuantityChange(item.quantity + 1)}
                    ...
                    <div className="flex justify-between sm:flex-col sm:items-end gap-3 sm:gap-2">
                        ...
                        <button
                            ...
                            disabled={isRemoving}
                            className="text-gray-400 hover:text-red-500 transition-colors disabled:opacity-50 p-2 hover:bg-red-50 rounded"
